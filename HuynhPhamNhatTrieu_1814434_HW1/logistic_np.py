...
def normalize_per_pixel(train_x, test_x):
    ...
    # Caculate x_mean and std for each pixel
    x_mean_per_pixel = np.mean(train_x, axis=0)
    std_per_pixel = np.sqrt(np.mean((train_x - x_mean_per_pixel)**2, axis=0))

    # Data normalizing
    train_x = (train_x - x_mean_per_pixel)/std_per_pixel
    test_x = (test_x - x_mean_per_pixel)/std_per_pixel
...
def normalize_all_pixel(train_x, test_x):
    ...
    # Caculate x_mean and std for all pixels
    x_mean_all_pixel = np.mean(train_x)
    std_all_pixel = np.sqrt(np.mean((train_x - x_mean_all_pixel)**2))

    # Data normalizing
    train_x = (train_x - x_mean_all_pixel)/std_all_pixel
    test_x = (test_x - x_mean_all_pixel)/std_all_pixel
...
def test(y_hat, test_y):
    ...
    # Caculate TP
    true_pos = sum((y_pred == 1) & (test_y == 1))

    # Caculate FP
    false_pos = sum((y_pred == 1) & (test_y == 0))
    ...
    # Caculate recall
    pos = sum(test_y)
    recall = float(np.sum(true_pos)) / float(np.sum(pos))
